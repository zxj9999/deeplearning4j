...
template <typename T>
static void getMKLDNNMemoryDescBatchNorm(const NDArray<T>* src, const NDArray<T>* diff_src,
...
#ifdef HAVE_MKLDNN
    if (block.isUseMKLDNN() && nd4j::MKLDNNStream<T>::isSupported() && numOfAxes == 1) {
        std::vector<nd4j::MKLDNNStream<T> >& streams = block.getMKLDNNStreams();
        if (streams.empty()) {
            streams.push_back(MKLDNNStream<T>("batchnorm_new"));
        }

        std::vector<Nd4jLong> shape({2, mean->lengthOf()});
        NDArray<T> weights('c', shape, block.getWorkspace());
        ...
        if (streams[0].checkAndReset({input, mean, variance, gamma, beta}, {output}, {epsilon}, axes)) {
            ...
            getMKLDNNMemoryDescBatchNorm<T>(input, nullptr, &batchnorm_src_md, nullptr, axes[0]);
