...
CUSTOM_OP_IMPL(conv3dnew, 2, 1, false, 0, 13) {
    ...
    if (block.isUseMKLDNN() && nd4j::MKLDNNStream<T>::isSupported()) {
        ...
        if (streams.empty()) {
            streams.push_back(MKLDNNStream<T>("conv3dnew"));
        }
        ...
        if (streams[0].checkAndReset({input, weights, bias}, {output}, {}, {kD, kH, kW, sD, sH, sW, pD, pH, pW, dD, dH, dW, isSameMode, isNCDHW})) {
            ...
            ConvolutionUtils<T>::getMKLDNNMemoryDescConv3d(kD, kH, kW, sD, sH, sW, pD, pH, pW, dD, dH, dW, isSameMode, isNCDHW,
...
CUSTOM_OP_IMPL(conv3dnew_bp, 3, 2, false, 0, 13) {
    ...
    if (block.isUseMKLDNN() && nd4j::MKLDNNStream<T>::isSupported()) {
        std::vector<nd4j::MKLDNNStream<T> >& streams = block.getMKLDNNStreams();
        if (streams.empty()) {
            streams.push_back(MKLDNNStream<T>("conv3dnew_bp_weights"));
            streams.push_back(MKLDNNStream<T>("conv3dnew_bp_data"));
        }
        ...
        if (resetW || resetI) {
            ...
            ConvolutionUtils<T>::getMKLDNNMemoryDescConv3d(kD, kH, kW, sD, sH, sW, pD, pH, pW, dD, dH, dW, isSameMode, isNDHWC,
