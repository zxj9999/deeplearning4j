...
template <typename T>
void ConvolutionUtils<T>::getMKLDNNMemoryDescConv3d(
        int kD, int kH, int kW, int sD, int sH, int sW, int pD, int pH, int pW, int dD, int dH, int dW, bool isSameMode, bool isNCDHW,
        int bS, int iC, int iD, int iH, int iW, int oC, int oD, int oH, int oW, const NDArray<T>* src, const NDArray<T>* diff_src,
        const NDArray<T>* weights, const NDArray<T>* diff_weights, const NDArray<T>* bias, const NDArray<T>* dst,
        mkldnn::memory::desc* conv_src_md, mkldnn::memory::desc* conv_diff_src_md, mkldnn::memory::desc* conv_weights_md,
        mkldnn::memory::desc* conv_diff_weights_md, mkldnn::memory::desc* conv_bias_md, mkldnn::memory::desc* conv_dst_md,
        mkldnn::memory::dims& conv_strides, mkldnn::memory::dims& conv_padding, mkldnn::memory::dims& conv_padding_r) {
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void ConvolutionUtils<T>::conv2d(nd4j::graph::Context<T>& block, const std::vector<NDArray<T>*>& inArrs, NDArray<T>* output, const std::vector<int>& intArgs) {
    ...
    if (block.isUseMKLDNN() && nd4j::MKLDNNStream<T>::isSupported()) {
        ...
        if (streams.empty()) {
            streams.push_back(MKLDNNStream<T>("conv2d"));
        }

        if (streams[0].checkAndReset(inArrs, {output}, {}, intArgs)) {
            ...
            getMKLDNNMemoryDescConv2d(kH, kW, sH, sW, pH, pW, dH, dW, isSameMode, isNCHW,
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void ConvolutionUtils<T>::conv2dBP(nd4j::graph::Context<T>& block, const std::vector<NDArray<T>*>& inArrs, const std::vector<NDArray<T>*>& outArrs, const std::vector<int>& intArgs) {
    ...
    if (block.isUseMKLDNN() && nd4j::MKLDNNStream<T>::isSupported()) {
        std::vector<nd4j::MKLDNNStream<T> >& streams = block.getMKLDNNStreams();
        if (streams.empty()) {
            streams.push_back(MKLDNNStream<T>("conv2d_bp_weights"));
            streams.push_back(MKLDNNStream<T>("conv2d_bp_data"));
        }

        bool resetW = streams[0].checkAndReset(inArrs, outArrs, {}, intArgs);
        bool resetI = streams[1].checkAndReset(inArrs, outArrs, {}, intArgs);
        if (resetW || resetI) {
            mkldnn_memory_desc_t empty;
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void ConvolutionUtils<T>::sconv2d(nd4j::graph::Context<T>& block, const std::vector<NDArray<T>*>& inArrs, NDArray<T>* output, const std::vector<int>& intArgs) {
    ...
    if (weightsPoint) {
        ConvolutionUtils<T>::conv2d(block, {outputDepth, weightsPoint, bias}, output, {1,1, 1,1, 0,0, 1,1, isSameMode, isNCHW});             // in this case oH=iH, oW=iW                
        delete outputDepth;
    }
}
...
template <typename T>
void ConvolutionUtils<T>::getMKLDNNMemoryDescPool3d(
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void ConvolutionUtils<T>::pooling3d(nd4j::graph::Context<T>& block, NDArray<T>& input, NDArray<T>& output, const T* extraParams) {
    ...
    if (poolingMode < 2 && block.isUseMKLDNN() && nd4j::MKLDNNStream<T>::isSupported()) {
        std::vector<nd4j::MKLDNNStream<T> >& streams = block.getMKLDNNStreams();
        if (streams.empty()) {
            streams.push_back(MKLDNNStream<T>("pooling3d"));
        }

        if (streams[0].checkAndReset({&input}, {&output}, std::vector<T>(extraParams, extraParams + 14), {})) {
            mkldnn_memory_desc_t empty;
            mkldnn::memory::desc pool_src_md(empty), pool_dst_md(empty);
            mkldnn::memory::dims pool_strides, pool_kernel, pool_padding, pool_padding_r;
            mkldnn::algorithm algorithm;

            getMKLDNNMemoryDescPool3d(kD, kH, kW, sD, sH, sW, pD, pH, pW, dD, dH, dW, poolingMode, (int)extraParam0, true,
...
void ConvolutionUtils<T>::pooling2dBP(nd4j::graph::Context<T>& block, NDArray<T>& input, NDArray<T>& gradO, NDArray<T>& gradI, const T* extraParams) {
    ...
    if (poolingMode < 2 && block.isUseMKLDNN() && nd4j::MKLDNNStream<T>::isSupported()) {
        ...
        if (streams[0].checkAndReset({&input, &gradO}, {&gradI}, std::vector<T>(extraParams, extraParams + 10), {})) {
            ...
            auto pool_desc = pooling_forward::desc(prop_kind::forward, algorithm, input.buffer() != nullptr ? pool_src_md : pool_diff_src_md, pool_dst_md,
                    pool_strides, pool_kernel, pool_padding, pool_padding_r, padding_kind::zero);
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void ConvolutionUtils<T>::pooling3dBP(nd4j::graph::Context<T>& block, NDArray<T>& input, NDArray<T>& gradO, NDArray<T>& gradI, const T* extraParams) {
    ...
    if (poolingMode < 2 && block.isUseMKLDNN() && nd4j::MKLDNNStream<T>::isSupported()) {
        std::vector<nd4j::MKLDNNStream<T> >& streams = block.getMKLDNNStreams();
        if (streams.empty()) {
            streams.push_back(MKLDNNStream<T>("pooling3d_bp"));
        }

        if (streams[0].checkAndReset({&input, &gradO}, {&gradI}, std::vector<T>(extraParams, extraParams + 14), {})) {
            mkldnn_memory_desc_t empty;
            mkldnn::memory::desc pool_src_md(empty), pool_diff_src_md(empty), pool_dst_md(empty);
            mkldnn::memory::dims pool_strides, pool_kernel, pool_padding, pool_padding_r;
            mkldnn::algorithm algorithm;
